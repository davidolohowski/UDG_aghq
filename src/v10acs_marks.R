...
GC_color <- readRDS('data/v10_acs_col.RDS')
X <- c(1, 4095, 4214, 223)
...
gal <- (loc$x - 3275)^2/1.42^2 + (loc$y - 4065)^2
# spde stuff
nu <- 1
...
post_node <- LGCP_postsamples$samps[5:1456,]
...
for (i in 1:50) {
  ...
  post_node0.0 <- samps0.0$samps[5:1456,]
  ...
  post_node1.0 <- samps1.0$samps[8:1459,]
...
saveRDS(exc_prob, 'LGCP_v10acs_cov_spde_marks/exc_prob_v10acs.RDS')
