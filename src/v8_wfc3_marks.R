...
GC_color <- readRDS('data/v8_wfc3_col.RDS')
X <- c(1, 4129, 4126, 40)
Y <- c(1, 233, 4382, 4177)
...
gal1 <- ((loc$x - 1490)*cos(3*pi/4) - (loc$y-396)*sin(3*pi/4))^2 + ((loc$x - 1490)*sin(3*pi/4) + (loc$y-396)*cos(3*pi/4))^2/2.677^2
gal2 <- (loc$x - 474)^2 + (loc$y - 203)^2
...
input_data <- list(Apixel = A.pp,
                   ...
                   priormean_beta = c(0,0),
                   priorsd_beta = c(sqrt(1000), sqrt(1000)),
                   priormean_R = c(5.257, 6.61),
                   priorsd_R = c(0.25, 0.25),
                   prior_rho_min = 400,
                   prior_rho_prob = 0.5,
                   prior_sigma_max = 1.5,
...
#saveRDS(LGCP_fit_no_marks,file = "LGCP_v8wfc3_cov_spde_marks/LGCP_v8_non_marked.RDS")
...
post_node <- LGCP_postsamples$samps[8:1444,]
...
input_data <- list(Apixel = A.pp,
                   ...
                   priormean_beta_m = c(0,0),
                   priorsd_beta_m = c(sqrt(1000),sqrt(1000)),
                   priormean_R_m = c(5.257, 6.61),
                   priorsd_R_m = c(0.25,0.25),
                   priormean_alpha = 1,
...
aghqtime <- difftime(Sys.time(),tm,units = 'secs')
#saveRDS(LGCP_fit_weight,file = "LGCP_v8wfc3_cov_spde_marks/LGCP_v8_weighted_marks.RDS")
cat("AGHQ took: ",format(aghqtime),"\n")
...
aghqtime <- difftime(Sys.time(),tm,units = 'secs')
#saveRDS(LGCP_fit_marked,file = "LGCP_v8wfc3_cov_spde_marks/LGCP_v8_marked.RDS")
cat("AGHQ took: ",format(aghqtime),"\n")
...
for (i in 1:50) {
  ...
  post_node <- LGCP_postsamples$samps[15:1451,]
  
  post_U <- (exp(Apix %*% post_node))
  sigma <- exp(LGCP_postsamples$thetasamples[[2]])
  
  df$mean <- rowMeans(post_U)
  
  # excursion functions for the weighted model
  df$exc0.5 <- excursions.mc(post_U, 0.5, 1, '>')$`F`
  df$exc0.75 <- excursions.mc(post_U, 0.5, exp(quantile(rnorm(1000, 0, sigma), 0.75)), '>')$`F`
  df$exc0.9 <- excursions.mc(post_U, 0.5, exp(quantile(rnorm(1000, 0, sigma), 0.9)), '>')$`F`
  df$exc0.95 <- excursions.mc(post_U, 0.5, exp(quantile(rnorm(1000, 0, sigma), 0.95)), '>')$`F`
  
  # samples from the non-marked model
  samps0.0 <- sample_marginal(LGCP_fit_no_marks, 1000)
  post_node0.0 <- samps0.0$samps[8:1444,]
  ...
  post_node1.0 <- samps1.0$samps[15:1451,]
...
saveRDS(exc_prob, 'LGCP_v8wfc3_cov_spde_marks/exc_prob_v8wfc3.RDS')
